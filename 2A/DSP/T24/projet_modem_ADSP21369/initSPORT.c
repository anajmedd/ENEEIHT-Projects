...
// Block_A is filled first, Block_B is sent
...
void InitSPORT()
{
    //Proceed from Block A to Block A
    TCB_Block_A[0] = (int) TCB_Block_A + 3 - OFFSET + PCI ;
    TCB_Block_A[3] = (unsigned int) Block_A - OFFSET ;

    //Proceed from Block B to Block B
    TCB_Block_B[0] = (int) TCB_Block_B + 3 - OFFSET + PCI ;
    TCB_Block_B[3] = (unsigned int) Block_B - OFFSET ;

    //Proceed from Block C to Block C
    TCB_Block_C[0] = (int) TCB_Block_C + 3 - OFFSET + PCI ;
    ...
    *pSPCTL0 =  0X02 | (OPMODE | SLEN24 | SPEN_A | SCHEN_A | SDEN_A);
    ...
    *pCPSP0A = (unsigned int) TCB_Block_A - OFFSET + 3 ;
    ...
    #ifdef DAC1
    *pSPCTL1 = 0X02 | (SPTRAN | OPMODE | SLEN24 | SPEN_A | SCHEN_A | SDEN_A) ;
    // write to DAC1
    *pCPSP1A = (unsigned int) TCB_Block_B - OFFSET + 3 ;
    #endif
    ...
    *pSPCTL2 = (SPTRAN | OPMODE | SLEN24 | SPEN_A | SCHEN_A | SDEN_A) ;
    // write to DAC3
    *pCPSP2A = (unsigned int) TCB_Block_B - OFFSET + 3 ;
    #endif

    #ifdef DAC4
    *pSPCTL2 |= (SPTRAN | OPMODE | SLEN24 | SPEN_B | SCHEN_B | SDEN_B) ;
    // write to DAC4
    *pCPSP2B = (unsigned int) TCB_Block_B - OFFSET + 3 ;
    #endif
