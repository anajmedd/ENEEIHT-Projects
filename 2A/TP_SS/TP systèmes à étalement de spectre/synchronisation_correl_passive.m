...
% % L'ajout du bruit blanc gaussien
 ...
 signal_recu=filter(fliplr(PNF),1,signal_emis_bruit);
 % Estimateur de l'instant optimal d'echantillonnage
 for kk=1:M
      signal__ech_qpsk=signal_recu(kk:M:end);
      p(kk)=sum(abs(signal__ech_qpsk))^2;
  end 
  [maximum,id_max]=max(p);

 t0=id_max;
 % Échantillonnge
 signal_ech_qpsk=signal_recu(length(PNF):t0:end);
 % Demodulation
 bits_decide=qamdemod(signal_ech_qpsk.',4,'OutputType','bit').';
