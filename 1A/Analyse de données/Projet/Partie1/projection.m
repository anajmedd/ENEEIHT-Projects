...
Wcp = [individu_moyen',W(:,1:n-1)];
C = X_centre * Wcp;

 for q = 0:n-1
     Cq = C(:,1:q);		% q premières composantes principales
     Eigenfaces = Wcp(:,1:q);		                        % q premières eigenfaces
     X_reconstruit = (Cq * pinv(Eigenfaces)+ ones(n,1)*individu_moyen)';
     figure(1);
     ...
     RMSE = sqrt(sum(sum(X'-X_reconstruit).^2))/length(X_reconstruit(1,:))*length((X_reconstruit(:,1)));
